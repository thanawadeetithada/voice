        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-6 pb-20">
            <div class="space-y-6">
                <div class="flex justify-between items-center">
                    <h1 class="text-2xl font-bold text-slate-800">จัดการผู้ใช้งาน (Settings)</h1>
                </div>
                        ...
                        <table class="w-full text-left border-collapse min-w-[700px]">
                            <thead>
                                <tr class="text-xs text-slate-500 uppercase tracking-wider bg-slate-50 border-b border-slate-200">
                                    ...
                                    <th class="p-4 font-semibold">User Role</th>
                            ...
                            <tbody>
                                ...
                                <tr class="border-b border-slate-50 hover:bg-emerald-50/30">
                                    <td class="p-4 text-sm text-slate-500 text-center"><?php echo $row['id']; ?></td>
                                    <td class="p-4 text-sm font-bold text-slate-700"><?php echo htmlspecialchars($row['username']); ?></td>
                                    <td class="p-4">
                                        <?php if($row['userrole'] == 'admin'): ?>
                                            <span class="px-3 py-1 rounded-full text-xs font-bold border bg-purple-50 text-purple-600 border-purple-200">Admin</span>
                                        <?php else: ?>
                                            <span class="px-3 py-1 rounded-full text-xs font-bold border bg-blue-50 text-blue-600 border-blue-200">User</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="p-4 text-center space-x-2">
                                        <a href="edit_data.php?id=<?php echo $row['id']; ?>" class="inline-flex items-center justify-center p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors" title="แก้ไขข้อมูล">
                                            <i data-lucide="edit" class="w-4 h-4"></i>
                                        </a>
                                        <button type="button" onclick="openDeleteModal(<?php echo $row['id']; ?>)" class="inline-flex items-center justify-center p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors" title="ลบผู้ใช้">
                                ...
                                ?>
                                    <tr><td colspan="4" class="p-4 text-center text-slate-500">ไม่มีข้อมูลผู้ใช้งาน</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-slate-900">ยืนยันการลบผู้ใช้งาน</h3>
                            <div class="mt-2">
                                <p class="text-sm text-slate-500">คุณแน่ใจหรือไม่ที่จะลบผู้ใช้งานรายนี้? <br>การกระทำนี้ไม่สามารถเรียกคืนข้อมูลได้</p>
                            </div>
                        </div>
                    </div>
                </div>
