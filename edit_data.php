...
<body class="bg-slate-50 font-sans text-slate-800 h-screen flex overflow-hidden">
    ...
    <div id="sidebar" class="fixed lg:static inset-y-0 left-0 z-30 w-64 bg-white border-r border-slate-200 flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            <a href="admin_dashboard.php" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-slate-500 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                <i data-lucide="layout-dashboard" class="w-5 h-5"></i> Dashboard
            </a>
            <a href="admin_tickets.php" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-slate-500 hover:bg-emerald-50 hover:text-emerald-700 transition-colors">
                <i data-lucide="inbox" class="w-5 h-5"></i> Tickets Inbox
            </a>
            <a href="admin_settings.php" class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium bg-emerald-600 text-white shadow-md shadow-emerald-200">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden w-full relative">
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-10">
            <div class="flex items-center gap-3">
                <a href="admin_settings.php" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-emerald-600 transition-colors font-medium">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-6 pb-20 flex justify-center items-start pt-10">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 max-w-lg w-full space-y-6">
                ...
                <div class="p-4 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm flex items-center gap-3">
                ...
                <form id="editForm" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">เปลี่ยนรหัสผ่านใหม่ (เว้นว่างไว้หากไม่ต้องการเปลี่ยน)</label>
                        <div class="relative">
                            ...
                            <button type="button" id="toggle_pass_btn" onclick="togglePassword('password', 'toggle_pass_btn')" class="absolute right-4 top-2.5 text-slate-400 hover:text-emerald-600 transition-colors">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="password" id="confirm_password" name="confirm_password" placeholder="••••••••••••••" 
                                   class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all pr-12">
                            <button type="button" id="toggle_confirm_btn" onclick="togglePassword('confirm_password', 'toggle_confirm_btn')" class="absolute right-4 top-2.5 text-slate-400 hover:text-emerald-600 transition-colors">
                                <i data-lucide="eye" class="w-5 h-5"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">สิทธิ์ผู้ใช้งาน (User Role)</label>
                        <select id="input_role" name="userrole" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all">
                            <option value="user" <?php echo ($user_data['userrole'] == 'user') ? 'selected' : ''; ?>>User (ผู้ใช้ทั่วไป)</option>
                            <option value="admin" <?php echo ($user_data['userrole'] == 'admin') ? 'selected' : ''; ?>>Admin (ผู้ดูแลระบบ)</option>
                        </select>
                    </div>

                    <div class="pt-4 flex gap-3">
                        <button type="button" onclick="openConfirmModal()" class="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold text-sm transition-colors shadow-sm shadow-emerald-100">
                            บันทึกการเปลี่ยนแปลง
                        </button>
                        <a href="admin_settings.php" class="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl font-semibold text-sm transition-colors text-center">
                            ยกเลิก
                        </a>
                    </div>
                </form>
    ...
    <div id="confirmModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 backdrop-blur-sm transition-opacity px-4">
        <div class="relative bg-white rounded-2xl max-w-sm w-full p-6 text-center shadow-xl transform transition-all">
            ...
            <p class="text-sm text-slate-500 mb-6">คุณต้องการอัปเดตข้อมูลของ <br><span id="show_modal_user" class="font-bold text-emerald-600"></span> ใช่หรือไม่?</p>
            
            <div class="flex gap-3">
                <button type="button" onclick="closeConfirmModal()" class="flex-1 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-sm transition-colors">
                    ยกเลิก
                </button>
                <button type="button" onclick="submitRealForm()" class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl text-sm transition-colors shadow-sm shadow-emerald-200">
                    ยืนยันบันทึก
                </button>
            </div>
        </div>
